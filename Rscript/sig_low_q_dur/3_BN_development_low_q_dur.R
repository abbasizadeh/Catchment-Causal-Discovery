...
# transform to normal
# write.csv(x = selected_data_slope_fdc, file = './results_text/low_q_dur/data_low_q_dur.csv')
...
pc.fit <- set.arc(pc.fit, from = 'geol_porosity', to = 'pet_mean')
pc.fit <- set.arc(pc.fit, from = 'geol_porosity', to = 'frac_forest')

pc.fit <- set.arc(pc.fit, from = 'frac_forest', to = 'lai_diff')

pc.fit <- set.arc(pc.fit, from = 'elev_mean', to = 'clay_frac')

pc.fit <- set.arc(pc.fit, from = 'gauge_lon', to = 'elev_mean')
pc.fit <- set.arc(pc.fit, from = 'gauge_lon', to = 'lai_diff')
pc.fit <- set.arc(pc.fit, from = 'gauge_lon', to = 'high_prec_dur')

pc.fit <- set.arc(pc.fit, from = 'clay_frac', to = 'frac_forest')
pc.fit <- drop.arc(pc.fit, from = 'clay_frac', to = 'high_prec_freq')

pc.fit <- set.arc(pc.fit, from = 'max_water_content', to = 'low_q_dur')

pc.fit <- set.arc(pc.fit, from = 'low_prec_dur', to = 'max_water_content')

pc.fit <- set.arc(pc.fit, from = 'low_prec_freq', to = 'frac_forest')


pc.fit <- set.arc(pc.fit, from = 'high_prec_freq', to = 'low_prec_freq')
pc.fit <- set.arc(pc.fit, from = 'high_prec_freq', to = 'low_prec_dur')

pc.fit <- set.arc(pc.fit, from = 'p_seasonality', to = 'high_prec_dur')
