################################################################################
#                                                                              #
#                       learn causal graph for low q dur                      #
#                                PC Algorithm                                  #
# ##############################################################################
...
parents(x = pc.fit, node = 'low_q_dur')
graphviz.plot(pc.fit)

saveRDS(object = pc.fit, file = './data/output/PC_DAGs/pc_mi_g_sh_lqd.rds')
graphviz.plot(readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_lqd.rds'))
...
saveRDS(object = pc_boot_strength, file = './data/output/PC_DAGs/boot_pc_mi_g_sh_lqd.rds')
...
dt_hqd_reg_test <- 
  readRDS(file = './data/output/regression_test/likelihood_ratio_test_low_q_dur')
...
# CPDAG to DAG-----------------------------------------------------------------
pc.fit <- readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_lqd.rds')
parents(x = pc.fit, node = 'low_q_dur')
plot(pc.fit)
pc.fit
...
pc.fit
saveRDS(object = pc.fit, file = './data/output/PC_DAGs/DAG_lqd.rds')
# readRDS(file = './data/output/PC_DAGs/DAG_lqd.rds')
plot(pc.fit)
