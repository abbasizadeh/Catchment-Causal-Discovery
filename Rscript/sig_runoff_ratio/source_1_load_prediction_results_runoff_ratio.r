...
selected_data_slope_fdc <- 
  merge(selected_data_slope_fdc, ALL_SIGNATURES[, c('gauge_id', 'runoff_ratio')], by = 'gauge_id')


# calculate number of clusters

folder <- 'sig_runoff_ratio'
target_node <- 'runoff_ratio'



source('./source/7_calculate_number_of_clusters.R')
# selected_data_slope_fdc


# whole data set prediction
prediction_results_whole_data <- 
  readRDS(paste0('./data/output/prediciton_results/prediction_results_whole_data_', target_node,'.rds'))
...
#   readRDS(
#     paste0(
#       './data/output/prediciton_results/prediction_results_predictors_clustering_',
#       target_node,
#       '.rds'
#     )
#   )
...
prediction_results_cluster_climate <-
  readRDS(
    paste0(
      './data/output/prediciton_results/prediction_results_climate_clustering_',
      target_node,
      '.rds'
    )
  )
...
for (i in cluster_number_climate[, n]){
  ...
  prediction_results_cluster_climate[
    cluster == clust_num, cluster_label := paste0('cluster ', cluster, '; ', i, ' catchments')]
...
prediction_results_cluster_geol <- 
  readRDS(paste0('./data/output/prediciton_results/prediction_results_geol_clustering_', target_node, '.rds'))
...
prediction_results_cluster_soil <-
  readRDS(
    paste0(
      './data/output/prediciton_results/prediction_results_soil_clustering_',
      target_node,
      '.rds'
    )
  )
...
prediction_results_cluster_topo <-
  readRDS(
    paste0(
      './data/output/prediciton_results/prediction_results_topo_clustering_',
      target_node,
      '.rds'
    )
  )
...
prediction_results_cluster_vege <-
  readRDS(paste0(
    './data/output/prediciton_results/prediction_results_vege_clustering_',
    target_node,
    '.rds'
  ))
...
#   readRDS(
#     paste0(
#       './data/output/prediciton_results/prediction_results_whole_data_',
#       target_node,
#       '.rds'
#     )
#   )
