...
var_import_plor <- 
  ggplot(ImpData, aes(x = Var.Names, y = `%IncMSE`)) +
    ...
    geom_point(aes(size = IncNodePurity, color = category, fill = category),
               alpha = 0.6, size = 10) +
  labs(title = 'Variable Importance: High Flow') + 
    theme_light() +
    coord_flip() +
    theme(
...
var_import_plor
ggsave(filename = './results_text/q95/8_q98_var_importance.png', 
...
corr_plot <-
  ggplot(cor_data[order(cor_coef), ]) +
    ...
    labs(x = "Variables", 
         ...
         title = "Pearson, Kendall, Spearman Correlations: High Flow") +
    theme_light() +
    scale_fill_manual(values = color_vac) +
...
corr_plot
ggsave(filename = './results_text/q95/8_q95_corr.png', 
