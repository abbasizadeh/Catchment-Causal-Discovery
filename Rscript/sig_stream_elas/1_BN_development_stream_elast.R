################################################################################
#                                                                              #
#                       learn causal graph for stream elast                    #
#                                PC Algorithm                                  #
# ##############################################################################
...
# run PC algorithms ---------------------------------------------------------------
set.seed(123)
pc.fit <- pc.stable(data_slope_fdc, test = 'mi-g-sh', blacklist = blacklist_pc, alpha = 0.15)
saveRDS(object = pc.fit, file = './data/output/PC_DAGs/pc_mi_g_sh_strm_elas.rds')
graphviz.plot(pc.fit)
parents(x = pc.fit, node = 'stream_elas')
graphviz.plot(readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_strm_elas.rds'))
...
saveRDS(object = pc_boot_strength, file = './data/output/PC_DAGs/boot_pc_mi_g_sh_srm_elas.rds')
...
dt_hqd_reg_test <- 
  readRDS(file = './data/output/regression_test/likelihood_ratio_test_stream_elas')
...
# CPDAG to DAG-----------------------------------------------------------------
pc.fit <- readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_strm_elas.rds')
plot(pc.fit)
parents(x = pc.fit, node = 'stream_elas')
pc.fit
...
saveRDS(object = pc.fit, file = './data/output/PC_DAGs/DAG_strm_elas.rds')
