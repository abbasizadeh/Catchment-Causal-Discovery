...
plot(rf_baseline)
hist(data_for_rf$slope_fdc)
summary(lm(rf_baseline$predicted ~ na.omit(data_for_rf)$slope_fdc))$r.squared
varImpPlot(rf_baseline)


# sorted tabular summary
rf_important_vars <- importance(rf_baseline) %>% 
  as.data.frame() %>% 
  arrange(desc(`%IncMSE`))
...
var_import_plor <-
  ggplot(ImpData, aes(x=Var.Names, y=`%IncMSE`)) +
    geom_segment(aes(x=Var.Names, xend=Var.Names, y=0, yend=`%IncMSE`), alpha = 0.2) +
    geom_point(aes(size = IncNodePurity, color = category, fill = category),  alpha=0.6) +
  labs(title = 'Variable Importance: Slope of FDC') + 
    theme_light() +
    coord_flip() +
    theme(
...
var_import_plor
ggsave(filename = './results_text/slope_fdc/10_slope_fdc_var_importance.png', 
...
corr_plot <-
  ggplot(cor_data[order(cor_coef), ]) +
    ...
    labs(x = "Variables", 
         ...
         title = "Pearson, Kendall, Spearman Correlations: Slope of FDC") +
    theme_light() +
    scale_fill_manual(values = color_vac) +
...
corr_plot
ggsave(filename = './results_text/slope_fdc/10_slope_fdc_corr.png', 
...
corr_results
write.csv(x = corr_results, file = './results_text/slope_fdc/corr.csv')
...
rm(cor_data, cor_data_kendall, cor_data_pearson, cor_data_spearman, 
   cor_kendall_imporant, cor_pearson_imporant, cor_spearman_imporant, 
   cor_kendall_matrix, cor_pearson_matrix, cor_spearman_matrix, data_for_rf,
   ImpData, rf_baseline, rf_important_vars, climate_varsm, color_vac,  geol_vars,
   hydro_vars, rf_imporant, soil_vars, target_column, topo_vars, vege_vars,
   climate_vars
   )
...
# feature plot
featurePlot(x = camels_hydro$slope_fdc,
            y = camels_clim[,-c('gauge_id',
                                'high_prec_timing',
                                'low_prec_timing')],
            plot = "pairs",
            type = c("p", "smooth"),
...
# # random forest
# rf.fit <- randomForest(
#   slope_fdc ~ ., data = na.omit(corr_matrix_data),
#   ntree = 1000, keep.forest = FALSE, importance = TRUE
# )
#
# summary(lm(rf.fit$predicted ~ na.omit(corr_matrix_data)$slope_fdc))$r.squared
#
# plot(rf.fit$predicted, na.omit(corr_matrix_data)$slope_fdc)
