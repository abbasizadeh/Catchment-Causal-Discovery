################################################################################
#                                                                              #
#                       learn causal graph for high q dur                      #
#                                PC Algorithm                                  #
# ##############################################################################
...
# run algorithms ---------------------------------------------------------------
set.seed(123)
pc.fit <- pc.stable(data_slope_fdc, test = 'mi-g-sh', blacklist = blacklist_pc, alph = 0.2)
parents(x = pc.fit, node = 'high_q_dur')
graphviz.plot(pc.fit)
saveRDS(object = pc.fit, file = './data/output/PC_DAGs/pc_mi_g_sh_hqd.rds')
graphviz.plot(readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_hqd.rds'))
...
saveRDS(object = pc_boot_strength, file = './data/output/PC_DAGs/boot_pc_mi_g_sh_hqd.rds')
...
dt_hqd_reg_test <- 
  readRDS(file = './data/output/regression_test/likelihood_ratio_test_high_q_dur')
...
# CPDAG to DAG-----------------------------------------------------------------
pc.fit <- readRDS(file = './data/output/PC_DAGs/pc_mi_g_sh_hqd.rds')
parents(x = pc.fit, node = 'high_q_dur')
plot(pc.fit)
...
pc.fit
saveRDS(object = pc.fit, file = './data/output/PC_DAGs/DAG_hqd.rds')
