...
var_import_plor <-   
  ggplot(ImpData, aes(x = Var.Names, y = `%IncMSE`)) +
    ...
    geom_point(aes(size = IncNodePurity, color = category, fill = category),
               alpha = 0.6, size = 10) +
    labs(title = 'Variable Importance: High Flow Durtion') +
    theme_light() +
    coord_flip() +
    theme(
...
var_import_plor
ggsave(filename = './results_text/high_q_dur/2_high_q_dur_var_importance.png', 
...
corr_plot <-
  ggplot(cor_data[order(cor_coef), ]) +
    ...
    labs(x = "Variables", 
         ...
         title = "Pearson, Kendall, Spearman Correlations: High Flow Duration") +
    theme_light() +
    scale_fill_manual(values = color_vac) +
...
corr_plot
ggsave(filename = './results_text/high_q_dur/2_high_q_dur_corr.png', 
...
corr_results
write.csv(x = corr_results, file = './results_text/high_q_dur/corr.csv')
