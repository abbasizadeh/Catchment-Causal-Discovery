...
# transform to normal
# write.csv(x = selected_data_slope_fdc, file = './results_text/high_q_dur/data_high_q_dur.csv')
...
pc.fit <- set.arc(pc.fit, from = 'lai_max', to = 'high_q_dur')

pc.fit <- set.arc(pc.fit, from = 'frac_forest', to = 'lai_max')

pc.fit <- set.arc(pc.fit, from = 'area_gages2', to = 'frac_forest')
pc.fit <- set.arc(pc.fit, from = 'area_gages2', to = 'p_mean')
pc.fit <- set.arc(pc.fit, from = 'area_gages2', to = 'soil_depth_pelletier')

# pc.fit <- set.arc(pc.fit, from = 'frac_forest', to = 'lai_diff')
# pc.fit <- set.arc(pc.fit, from = 'geol_porosity', to = 'baseflow_index')

pc.fit <- set.arc(pc.fit, from = 'slope_mean', to = 'geol_porosity')
pc.fit <- set.arc(pc.fit, from = 'slope_mean', to = 'high_prec_dur')
pc.fit <- set.arc(pc.fit, from = 'slope_mean', to = 'soil_depth_pelletier')
pc.fit <- set.arc(pc.fit, from = 'slope_mean', to = 'frac_forest')
...
pc.fit <- set.arc(pc.fit, from = 'silt_frac', to = 'high_q_dur')
...
# pc.fit <- drop.arc(pc.fit, from = 'clay_frac', to = 'sand_frac')

pc.fit <- set.arc(pc.fit, from = 'soil_depth_pelletier', to = 'frac_forest')
pc.fit <- set.arc(pc.fit, from = 'soil_depth_pelletier', to = 'geol_porosity')
# pc.fit <- set.arc(pc.fit, from = 'sand_frac', to = 'geol_porosity')

pc.fit <- set.arc(pc.fit, from = 'low_prec_dur', to = 'lai_max')
pc.fit <- set.arc(pc.fit, from = 'low_prec_freq', to = 'frac_forest')

# pc.fit <- set.arc(pc.fit, from = 'frac_snow', to = 'baseflow_index')

pc.fit <- set.arc(pc.fit, from = 'p_mean', to = 'high_q_dur')
