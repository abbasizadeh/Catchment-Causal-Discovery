...
var_import_plor <- 
  ggplot(ImpData, aes(x = Var.Names, y = `%IncMSE`)) +
    ...
    geom_point(aes(size = IncNodePurity, color = category, fill = category),
               alpha = 0.6, size = 10) +
  labs(title = 'Variable Importance: Low Flow') + 
    theme_light() +
    coord_flip() +
    theme(
...
ggsave(filename = './results_text/q5/7_q5_var_importance.png', 
...
corr_plot <-
  ggplot(cor_data[order(cor_coef), ]) +
    ...
    labs(x = "Variables", 
         ...
         title = "Pearson, Kendall, Spearman Correlations: Q5") +
    theme_light() +
    scale_fill_manual(values = color_vac) +
...
corr_plot
ggsave(filename = './results_text/q5/7_q5_corr.png', 
...
corr_results
write.csv(x = corr_results, file = './results_text/q5/corr.csv')
