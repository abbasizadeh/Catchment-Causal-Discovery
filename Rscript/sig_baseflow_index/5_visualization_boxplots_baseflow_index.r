...
ggplot(data = visual_1, aes(x = variable,  y = value, fill = cal_val, color = cal_val)) +
  geom_boxplot(outlier.shape = NA, staplewidth = 0.75, size = 0.8) +
  scale_fill_manual(
    values = c('darkorange', 'deepskyblue')
  ) +
  scale_color_manual(
    values = c('darkorange4', 'deepskyblue4')
  ) +
  ...
  theme(title = element_text(size = 25),
        axis.text.x = element_text(
          angle = 90, vjust = 0.5, hjust = 1, size = 15, face = 'bold'),axis.title = element_text(size = 20),
        axis.text.y = element_text(size = 15, face = 'bold'),
        legend.title = element_blank(),
        legend.text = element_text(size = 15, face = 'bold'),
        legend.position = 'bottom', legend.background = element_rect(colour = 'azure3'), 
...
ggplot(data = visual_1_RMSE, aes(x = variable,  y = value, fill = cal_val)) +
  geom_boxplot(outlier.shape = NA, staplewidth = 0.75, size = 0.8) +
  scale_fill_manual(
    values = c('darkorange', 'deepskyblue')
  ) +
