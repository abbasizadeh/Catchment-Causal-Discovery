# working directory
# setwd("~/PhD/spatial_variability/modifed_R_scripts/")

target_node <- 'baseflow_index'

# whole dataset ----------------------------------------------------------------
whole_data_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_whole_clust_',
    target_node,
    '.csv'
  )
)

# whole_stat <- whole_data_stat[is.na(cluster), ]
whole_stat <- copy(whole_data_stat)
whole_stat[, V1 := NULL]

whole_stat <-
  whole_stat[, .(mean_whole_data = mean(mean), median_whole_data = mean(median)), 
             by = .(variable)]

# # predictors -------------------------------------------------------------------
# pred_stat <- fread(paste0('./data/output/summary_stat/baseflow_index/summary_stat_predictors_clust_', target_node,'.csv'))
# head(pred_stat)
# 
# pred_stat[, V1 := NULL]
# 
...
clim_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_climate_clust_',
    target_node,
    '.csv'
  )
)
...
clim_cluster_stat <-
  clim_cluster_stat[
    , .(weighted_mean = sum(mean * ratio), weighted_median = sum(median * ratio)),
    by = .(variable)]
...
write.csv(
  clim_results,
  file = paste0(
    './data/output/summary_stat/baseflow_index/weighted_climate_clust_overal_results_',
    target_node,
    '.csv'
  )
)
...
geol_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_geol_clust_',
    target_node,
    '.csv'
  )
)
...
write.csv(
  geol_results,
  file = paste0(
    './data/output/summary_stat/baseflow_index/weighted_geol_clust_overal_results_',
    target_node,
    '.csv'
  )
)
...
soil_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_soil_clust_',
    target_node,
    '.csv'
  )
)
...
write.csv(
  soil_results,
  file = paste0(
    './data/output/summary_stat/baseflow_index/weighted_soil_clust_overal_results_',
    target_node,
    '.csv'
  )
)
...
topo_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_topo_clust_',
    target_node,
    '.csv'
  )
)
...
topo_cluster_stat <- na.omit(topo_stat)

topo_cluster_stat <-
  topo_cluster_stat[, .(weighted_mean = sum(mean * ratio), weighted_median = sum(median * ratio)),
                    by = .(variable)]
...
# summary_stat_climate_overal_results.csv
write.csv(topo_results, 
          file = paste0('./data/output/summary_stat/baseflow_index/weighted_topo_clust_overal_results_', target_node, '.csv'))
...
vege_stat <- fread(
  paste0(
    './data/output/summary_stat/baseflow_index/summary_stat_vege_clust_',
    target_node,
    '.csv'
  )
)
...
write.csv(
  vege_results,
  file = paste0(
    './data/output/summary_stat/baseflow_index/weighted_vege_clust_overal_results_',
    target_node,
    '.csv'
  )
)
...
whole_data_stat[, ratio := as.numeric(ratio)]
whole_data_stat[, ratio := as.numeric(1)]


whole_data_stat[, V1 := NULL]

# whole_data_cluster_stat <- na.omit(whole_data_stat)
whole_data_cluster_stat <- copy(whole_data_stat)
...
write.csv(
  whole_data_results,
  file = paste0(
    './data/output/summary_stat/baseflow_index/weighted_whole_clust_overal_results_',
    target_node,
    '.csv'
  )
)
