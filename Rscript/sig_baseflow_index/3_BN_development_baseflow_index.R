...
# transform to normal
# write.csv(x = selected_data_slope_fdc, file = './results_text/baseflow_index/data_baseflow_inedx.csv')
...
pc.fit <- set.arc(pc.fit, from = 'clay_frac', to = 'baseflow_index')
...
pc.fit <- set.arc(pc.fit, from = 'sand_frac', to = 'baseflow_index')
...
graphviz.plot(pc.fit, shape = "circle", layout = "circo", fontsize = 40,
              highlight = list(
                nodes = parents(x = pc.fit, node = 'baseflow_index'), 
                col = "tomato", 
                fill = "orange", 
                arcs = incoming.arcs(pc.fit, "baseflow_index"),
                col = "darkblue", fill = "lightblue", lwd = 3),
              )
              

qgraph(
  pc.fit,
  title = "Estimated DAG For Baseflow Index",
  title.cex = 2,
  label.cex = 4,
  label.prop = 0,
  vsize = 8,
...
plot(pc.fit, highlight = "baseflow_index")
# parents(x = pc.fit, node = 'baseflow_index')
# edge strength
